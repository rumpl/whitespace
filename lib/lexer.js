...
Lexer.prototype.flowControl = function (line) {
  if (line.startsWith('  ')) {
    return { op: Token.MARK, label: this.parseNumber(line.substr(2)) };
  }

  if (line.startsWith(' \t')) {
    return { op: Token.CALL_SUBROUTINE, label: this.parseNumber(line.substr(2)) };
  }

  if (line.startsWith(' \n')) {
    return { op: Token.JUMP, label: this.parseNumber(line.substr(2)) };
  }

  if (line.startsWith('\t ')) {
    return { op: Token.JUMP_IF_ZERO, label: this.parseNumber(line.substr(2)) };
  }

  if (line.startsWith('\t\t')) {
    return { op: Token.JUMP_IF_NEGATIVE, label: this.parseNumber(line.substr(2)) };
  }
